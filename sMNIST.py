...
def train_model(net, optimizer, num_epochs):
    ...
    for epoch in range(0, num_epochs):
        ...
        if epoch % SAVEFREQ == 0 or epoch == num_epochs - 1:
            with open(SAVEDIR + '{}_Train_Losses.p'.format(NET_TYPE),
                      'wb') as fp:
                pickle.dump(train_losses, fp)

            with open(SAVEDIR + '{}_Test_Losses.p'.format(NET_TYPE),
                      'wb') as fp:
                pickle.dump(test_losses, fp)

            with open(SAVEDIR + '{}_Test_Accuracy.p'.format(NET_TYPE),
                      'wb') as fp:
                pickle.dump(test_accuracies, fp)

            with open(SAVEDIR + '{}_Train_Accuracy.p'.format(NET_TYPE),
                      'wb') as fp:
