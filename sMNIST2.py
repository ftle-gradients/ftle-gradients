...
parser.add_argument('--save-freq', type=int,
                    default=10, help='frequency to save data')
...
class Model(nn.Module):
    ...
    def forward(self, inputs, y, order, h=None):
        h = None
        inputs = inputs[:, order]
        for input in torch.unbind(inputs, dim=1):
            h = self.rnn_layer(input.unsqueeze(1), h)
        out = self.lin(h)
...
def test_model(net, dataloader, h=None):
    accuracy = 0
    loss = 0
    net.eval()
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            loss, c = net.forward(x, y, order, h)
            accuracy += c
...
def train_model(net, optimizer, num_epochs):
    ...
    for epoch in range(0, num_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if NET_TYPE == 'LSTM':
                net.rnn_layer.init_states(inp_x.shape[0])
            ...
            if NET_TYPE == 'nnRNN' and alam > 0:
                alpha_loss = net.rnn_layer.alpha_loss(alam)
                loss += alpha_loss
            ...
            if orthog_optimizer:
                net.rnn_layer.orthogonal_step(orthog_optimizer)
            optimizer.step()
        ...
        if epoch % SAVEFREQ == 0 or epoch == num_epochs - 1:
            with open(SAVEDIR + f'{NET_TYPE}_Train_Losses_e{epoch}',
                      'wb') as fp:
                pickle.dump(train_losses, fp)

            with open(SAVEDIR + f'{NET_TYPE}_Test_Losses_e{epoch}',
                      'wb') as fp:
                pickle.dump(test_losses, fp)

            with open(SAVEDIR + f'{NET_TYPE}_Test_Accuracy_e{epoch}',
                      'wb') as fp:
                pickle.dump(test_accuracies, fp)

            with open(SAVEDIR + f'{NET_TYPE}_Train_Accuracy_e{epoch}',
                      'wb') as fp:
...
if CUDA:
    net = net.cuda()
    print('Net moved')
    net.rnn_layer = net.rnn_layer.cuda()
    print('RNN layer moved')
print('sMNIST task')
print(NET_TYPE)
...
model_fname = '2023-04-26_09-50-27(tanh)'
if load_trained:
    model_ckpt =torch.load(f'SMNIST/Models/{model_fname}/best_model.pth.tar')
    net.load_state_dict(model_ckpt['state_dict'])
    loss, accuracy = test_model(net, testloader)
    print(f'Model loss: {loss}, Model Accuracy: {accuracy}')
    test_input = next(iter(testloader))[0][0, 0].view(784)[order]
    plt.imshow(test_input.view(28, 28))
    plt.show()
else:
