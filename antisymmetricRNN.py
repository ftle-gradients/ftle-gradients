...
class AntisymmetricRNN(nn.Module):
	def __init__(self, input_size, hidden_size=32, eps=0.01, gamma=0.01, init_W_std=1,
		...
		# self.params = nn.ParameterList([self.weight_ih_l0.weight, self.weight_W_l0])
		if self.bias:
			self.params.append(self.weight_ih_l0.bias)
	...
	def get_init(self, init_W_std):
		...
		weight_W_l0 = nn.parameter.Parameter(normal_sampler_W.sample((self.hidden_size, self.hidden_size))[..., 0])
		# bias_W_l0 = nn.parameter.Parameter(torch.zeros(self.hidden_size))
		# self.weight_W_l0 = nn.Linear(self.hidden_size, self.hidden_size)
		self.weight_W_l0 = weight_W_l0
		# if self.bias:
			# self.weight_W_l0.bias = bias_W_l0
